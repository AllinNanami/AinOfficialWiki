...
<Dropdown :items="[{ text: '鼠标悬停菜单项 A', action: () => {} }, { text: '鼠标悬停菜单项 B', action: () => {} }]" trigger="hover" placement="bottom">
  <template #trigger>
    <LinkButton text="Hover 触发" variant="outline" />
  </template>
</Dropdown>
...
<Dropdown :items="wideItems" placement="bottom-start" :min-width="260">
  <template #trigger>
    <LinkButton text="自定义最小宽度 (260px)" variant="outline" />
  </template>
</Dropdown>
...
<div class="vp-pro-dropdown-demo-row">
  <Dropdown :items="[{ text: 'Top 菜单位于上方', action: () => {} }]" placement="top">
    <template #trigger><LinkButton text="Top" variant="outline" /></template>
  </Dropdown>
  <Dropdown :items="[{ text: 'Bottom 菜单位于下方', action: () => {} }]" placement="bottom">
    <template #trigger><LinkButton text="Bottom" variant="outline" /></template>
  </Dropdown>
  <Dropdown :items="[{ text: 'Top Start', action: () => {} }]" placement="top-start">
    <template #trigger><LinkButton text="Top Start" variant="outline" /></template>
  </Dropdown>
  <Dropdown :items="[{ text: 'Bottom End', action: () => {} }]" placement="bottom-end">
    <template #trigger><LinkButton text="Bottom End" variant="outline" /></template>
  </Dropdown>
</div>
