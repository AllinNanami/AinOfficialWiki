...
规则文件也一样。`AGENTS.md`、`CLAUDE.md` 确实有用，但它们是文本约束。模型可能遵守，也可能漏掉，还可能在上下文漂移、任务切换、工具调用链很长的时候绕开。原始参考里说得很直：给 `rm` 戴垃圾桶头盔，和给 AI 上儿童锁差不多。能挡掉一部分意外，但挡不住所有路线。
...
原始参考里还有一句很重要：`CLAUDE.md / AGENTS.md 要当活文档维护。`
