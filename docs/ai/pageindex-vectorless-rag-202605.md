...
原始参考把它概括成"基于无向图（Vectorless）和推理（Reasoning）的问答系统（RAG）的文档索引"。结合 `VectifyAI/PageIndex` 的项目说明，更准确的理解是：它把长文档整理成一棵层级化目录树，再让模型顺着章节、子节点、页码和文内引用一路找下去。这里说的 `Vectorless`，重点落在两件事：不用向量数据库，也不把文档先切成一堆固定 chunk 再做相似度召回。
