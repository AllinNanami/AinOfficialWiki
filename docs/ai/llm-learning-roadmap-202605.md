...
把清单里的**社媒参考 / 用户复制原文**保留下来：

> 上海交大 Dive into LLMs，11 个主题全覆盖，每个模块都有课件 + 教程 + 可跑代码，不是纯理论。11 个方向：微调与部署、提示学习与思维链、知识编辑、数学推理 / 蒸馏迷你 R1、模型水印、越狱攻击、大模型隐写、多模态、GUI 智能体、智能体安全、RLHF 安全对齐。还联合华为昇腾出了配套课程，初中高三级，有视频有实验手册。
>
> minimind-o：从 0 训练 0.1B 能听能说能看的全模态 Omni 模型。minimind：2 小时从 0 训练 65M 参数的视觉多模态 VLM。minimind-v：2 小时完全从 0 训练 64M 的小参数 LLM。

上面第二段原文把 `minimind` 和 `minimind-v` 的定位写反了。按这次归档到本地的项目说明核对：

- `minimind` 是纯语言 LLM 仓库；
- `minimind-v` 是视觉语言模型 VLM 仓库；
- `minimind-o` 是文 / 音 / 图全模态 Omni 仓库。

这个差异我会在正文里纠正，但保留原文记录，方便后续回看来源层级。
...
它和用户复制原文里那句"能听能说能看"是对得上的，而且仓库里还给了技术报告链接，这意味着你可以一边跑工程，一边对照论文看架构和评估，不用只靠仓库介绍猜。
