...
原始参考先完整放在这里。这部分来自用户复制原文和社媒参考，后面再逐步拆解。

## 原始参考：DeepSeek 六步 SOP（用户复制原文 / 社媒参考）
...
原始参考要求收 5–10 篇同类素材，这个数量是合理的。太少，素材面不够；太多，整理成本又会暴涨。
...
原始参考里提到的小红书、公众号、知乎、YouTube 字幕，核心意思就是一句话：资料源不要太单一。文字、视频、帖子混起来，整理出来的东西才会像是真的读过一轮材料，不会只剩平台洗稿味。

## 第 3 步：把 AI 当资料整合器，不当第一作者

这是整套方法的中心。原文里那句"让 AI 做信息聚合而非创作"，值得原封不动地记住。
...
原始参考第 4 步写的是：口语化表达重写，去掉套话和万能金句，保留逻辑但换掉模板句式。这一步我赞成，但有个前提：**去 AI 味不等于伪装人工，也不等于改掉事实。**
...
原始参考里直接点名了五类内容：亲身经历、踩坑、私有数据、独到判断、反常识观点。我基本同意，因为一篇内容有没有作者自己的东西，很多时候就看这五类材料有没有进来。
...
原始参考把第 6 步写成"取精去糟、二次精炼"。这是对的。很多稿子在第 3 步之后已经有信息，在第 4 步之后也没那么僵，但还是难读，原因通常是删得不够。
...
原始参考里这段方法必须完整保留，因为它和前面的六步 SOP 不在同一层。前六步讲的是"写一篇内容怎么做"，这一段讲的是"平时怎么找题"。
...
### 列表链接和图片链接，这次拿到了什么，没拿到什么

用户要求保留 X 列表链接和图片链接。这一条需要说清楚：

- **图片链接**：这次回收到了。配图已经本地化到 `./assets/ai-content-creation-sop/astronaut-deepseek-post.jpg`。
- **原帖链接**：这次也回收到了。
  - SOP 帖子：<https://x.com/Astronaut_1216/status/2054910493148483738>
  - 被引用短帖：<https://x.com/Astronaut_1216/status/2054563816336228790>
- **具体 X 列表链接**：原始参考只说"准备经常发最新模型的 AI 博主列表"，但没有附上现成列表地址。本轮公开检索也没回收到一个明确可访问的列表链接，所以这里不能编造，只能如实记为待回访项。

![Astronaut_1216 原始短帖配图（已本地化）](./assets/ai-content-creation-sop/astronaut-deepseek-post.jpg)
