...
`matlab-agentic-toolkit` 是目前看到的**最完整的厂商官方领域 Skills 产品**。和 `academic-research-skills` 比，它不是社区项目，而是 MathWorks 官方维护；和 `mattpocock/skills` 比，它不是通用工程方法论，而是专攻 MATLAB + 工具箱这一个垂直领域。
...
- 如果做工程和科学计算，`matlab-agentic-toolkit` 是 MathWorks 官方的 MATLAB 领域方案；
- 如果有核心技术书想嵌入工作流，`book-to-skill` 能把书架变成工具箱；
