...
这股趋势有很直接的项目热度支撑。2026 年 5 月 16 日我核对 GitHub 时，三个相关项目的热度都已经不低：
...
这和当前很多 Agent 工作流的取向是一致的：与其纠结"AI 会不会点按钮"，不如把高频动作压成稳定命令。
...
比如设计、GIS、视频剪辑、桌面软件自动化这类场景，往往第一步最难。CLI-Anything 的价值就是把第一步拆出来做。
...
截至 2026 年 5 月 16 日，我核对项目文档时，官方写的是：
...
CLI 这波重新变热，核心原因是 Agent 工作流开始认真处理"执行接口"这件事。MCP 负责接入，Skill 负责流程，CLI 负责落地执行。三层分工摆正之后，就更容易判断：什么时候该接浏览器，什么时候该补命令面，什么时候该把结果直接写回协作系统。
