...
import { resetBrowserStackToRoot, resolveBrowserStack, resolveExpandedKeys } from '../../composables/sidebar-state'
...
describe('sidebar-normalize', () => {
  ...
  test('collects all expandable folder keys', () => {
    ...
    expect(collectExpandableKeys(nodes)).toEqual(['总览', 'ai', 'ai/vibecoding'])
  })
  ...
  test('resets browser stack to the current tab root page', () => {
    const nodes = normalizeSidebarTree(sidebar, '/guides/improving-frontend-design-through-skills')
    const currentStack = [
      { key: 'root', title: '菜单', nodes },
      { key: 'ai', title: 'AI', nodes: nodes[1]?.items ?? [] },
    ...
    expect(resetBrowserStackToRoot(currentStack).map((page) => page.key)).toEqual(['root'])
  })
})
