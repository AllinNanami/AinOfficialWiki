...
import { buildBreadcrumbTrail, normalizeDocPath, resolveDocDisplayTitle } from '../components/ui/breadcrumbs'
...
export function useDocBreadcrumbItems() {
  ...
  const breadcrumbItems = computed(() => {
    const title = resolveDocDisplayTitle({
      pageTitle: page.value.title || page.value.frontmatter?.title || rootLabel.value,
    ...
    return buildBreadcrumbTrail({
      routePath: routePath.value,
      pageTitle: title,
      sidebar: (theme.value.sidebar ?? {}) as any,
      rootLabel: rootLabel.value
    })
  })
